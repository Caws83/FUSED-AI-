...
export function TokenTerminal({
  ...
  const usdSpot = live?.ethUsd ?? ethUsd;
      ...
      <main className="fused-terminal-main">
        ...
        <Card>
          ...
          <div className="fused-token-stats">
            ...
            <TerminalStat
              label="Market cap"
              value={formatHeadlineUsd(mc, chainId, usdSpot)}
              hint={
                isUsdStableQuoteChain(chainId)
                  ? undefined
                  : formatNative(mc.toString(), quoteSymbol)
              }
            />

            <TerminalStat
              label="FDV"
              value={formatHeadlineUsd(fdv, chainId, usdSpot)}
              hint={
                isUsdStableQuoteChain(chainId)
                  ? undefined
                  : formatNative(fdv.toString(), quoteSymbol)
              }
            />
          ...
          <dl className="fused-review">
            ...
            {graduationTargetUsd ? (
              <div>
                ...
                <dd>
                  ~$
                  {graduationTargetUsd.toLocaleString()} (not a live price)
                </dd>
              </div>
            ) : null}
...
function TerminalStat({
  ...
  return (
    <div className="fused-terminal-stat">
      <span>{label}</span>
      <strong>{value}</strong>
      {hint ? <em className="fused-terminal-stat-hint">{hint}</em> : null}
    </div>
  );
}