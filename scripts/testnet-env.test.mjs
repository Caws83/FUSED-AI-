...
test("env:testnet prints public Vercel vars and no secrets", () => {
  ...
  assert.match(result.stdout, /DEFAULT_LAUNCH_VERSION=v2/);
  assert.match(result.stdout, /LAUNCH_FACTORY_ADDRESS=0x359b3D82d958488eA9177c0F56EB3558ba59a40B/);
  assert.match(result.stdout, /LAUNCH_LOCKER_ADDRESS=0x2De462b0a9A7bB378a8a4E68a352eF30A9250D15/);
  assert.match(result.stdout, /LAUNCH_FACTORY_V1_ADDRESS=0x42654079a991EE21e2d2f7Eed0A77bf6a0082208/);
  assert.match(result.stdout, /LAUNCH_LOCKER_V1_ADDRESS=0x68000CD8F3AFE93BB87BeEDc9f2daBbf39E0836b/);
  assert.match(result.stdout, /LAUNCH_FACTORY_V2_ADDRESS=0x359b3D82d958488eA9177c0F56EB3558ba59a40B/);
  assert.match(result.stdout, /INDEXER_START_BLOCK=117433209/);
